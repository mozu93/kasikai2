...
2. **GUIダイアログ**に従って進行
3. **Python環境チェック**（未インストールの場合は自動案内）
4. **ライブラリ自動インストール**
5. **フォルダ作成と初期設定**

#### ステップ2: 会議室設定（オプション）
1. セットアップ中に**設定エディター**が起動
2. **会議室名や表示項目**をGUIで設定
3. **「設定を保存」**をクリック

#### ステップ3: システム起動
1. セットアップ完了後、**自動的にシステム起動**
2. **ブラウザが自動で開く**: http://localhost:5003
3. **システムトレイにアイコン表示**（青いカレンダー）
...
3. **リアルタイム更新**: 10秒間隔でデータが自動更新される

### 新機能確認
1. **支払額合計**: サイドバー下部に金額合計が表示される
